...
- **밤샘 풀 솔브 전용 케이스**: "🌙 [서버 local·밤샘] IBC 3D 정밀 그리드" —
  서버 앱에서 이 케이스를 local로 실행해 두면 조합별 솔브 → *_solved.mph 저장 →
  지표·히트맵·J-V까지 전부 자동 완료됩니다 (조합 실패는 건너뛰고 계속, 취소 시
  완료분 보존). 아침에 ④에서 결과 확인. solved.mph를 이 PC로 가져와 ④에
  업로드해 재추출하는 것도 가능.
